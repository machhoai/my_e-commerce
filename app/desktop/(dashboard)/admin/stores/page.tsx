        ...
        <div className="space-y-6 mx-auto">
                ...
                titleChildren={
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
                        <div>
                            ...
                            <p className="text-surface-500 mt-1">Thêm, sửa, bật/tắt cửa hàng, văn phòng và kho tổng.</p>
                        </div>
                        <button
                            onClick={() => { resetForm(); setFormType(activeTab); setIsCreateOpen(true); }}
                            className="flex items-center gap-2 bg-accent-600 hover:bg-accent-700 text-white px-5 py-2.5 rounded-xl font-medium shadow-sm transition-colors shrink-0"
                        >
                ...
                {([
                    ...
                    >
                        ...
                        <span className={cn(
                            ...
                            activeTab === tab.key ? 'bg-accent-100 text-accent-700' : 'bg-surface-200 text-surface-500'
                        )}>{tab.count}</span>
                    </button>
                ))}
            </div> */}
            ...
            {!loading && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        { label: 'Tổng Địa điểm', value: tabStores.length, color: 'bg-accent-50 text-accent-700', icon: Building2 },
                        { label: 'Đang hoạt động', value: tabStores.filter(s => s.isActive).length, color: 'bg-success-50 text-success-700', icon: CheckCircle2 },
                        { label: 'Tạm dừng', value: tabStores.filter(s => !s.isActive).length, color: 'bg-surface-100 text-surface-600', icon: PowerOff },
                        { label: 'Tổng Nhân sự', value: tabStores.reduce((acc, s) => acc + (s.stats?.total ?? 0), 0), color: 'bg-warning-50 text-warning-700', icon: Users },
                    ].map(item => (
                    ...
                    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-lg font-bold text-surface-800">{editStore ? 'Sửa Cửa hàng' : 'Thêm Cửa hàng Mới'}</h2>
                                <button onClick={resetForm} className="p-1.5 hover:bg-surface-100 rounded-lg transition-colors">
                                    <X className="w-5 h-5 text-surface-500" />
                                </button>
                            </div>
                            <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-semibold text-surface-700 mb-1.5">Tên cửa hàng *</label>
                                    <input
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-surface-700 mb-1.5">Loại địa điểm</label>
                                    <select
                                        value={formType}
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-surface-700 mb-1.5">Địa chỉ</label>
                                    <input
                ...
                {loading ? (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-surface-500 bg-surface-50 border-b border-surface-200">
                                <tr>
                                    ...
                                    <th className="px-4 py-3.5 font-semibold text-center">
                                        <Crown className="w-3.5 h-3.5 inline mr-1 text-accent-500" />CH Trưởng
                                    </th>
                                    <th className="px-4 py-3.5 font-semibold text-center">
                                        <ShieldCheck className="w-3.5 h-3.5 inline mr-1 text-warning-500" />Quản lý
                                    </th>
                                    <th className="px-4 py-3.5 font-semibold text-center">
                                        <UserCheck className="w-3.5 h-3.5 inline mr-1 text-primary-500" />Nhân viên
                                    </th>
                            ...
                            <tbody className="divide-y divide-surface-100">
                                {tabStores.map(store => (
                                    <tr key={store.id} className="hover:bg-surface-50/60 transition-colors group">
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className={cn(
                                                    'w-9 h-9 rounded-xl flex items-center justify-center shrink-0',
                                                    store.isActive ? 'bg-accent-50' : 'bg-surface-100'
                                                )}>
                                                    <Store className={cn('w-4 h-4', store.isActive ? 'text-accent-600' : 'text-surface-400')} />
                                                </div>
                                                <div>
                                                    <p className={cn('font-semibold', !store.isActive && 'text-surface-400 line-through')}>
                                                        {store.name}
                                                    </p>
                                                    {store.address && (
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <span className={cn(
                                                ...
                                                store.isActive ? 'bg-success-50 text-success-700' : 'bg-surface-100 text-surface-500'
                                            )}>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            {statsLoading ? (
                                                <div className="w-4 h-4 border-2 border-surface-300 border-t-surface-500 rounded-full animate-spin mx-auto" />
                                            ) : (
                                                <span className="font-bold text-accent-700 bg-accent-50 rounded-full w-8 h-8 flex items-center justify-center mx-auto text-sm">
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            {statsLoading ? (
                                                ...
                                                <span className="font-bold text-warning-700 bg-warning-50 rounded-full w-8 h-8 flex items-center justify-center mx-auto text-sm">
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            {statsLoading ? (
                                                ...
                                                <span className="font-bold text-primary-700 bg-primary-50 rounded-full w-8 h-8 flex items-center justify-center mx-auto text-sm">
                                        ...
                                        <td className="px-4 py-4 text-right">
                                            <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    ...
                                                    className="p-2 hover:bg-accent-50 text-surface-500 hover:text-accent-600 rounded-lg transition-colors"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        store.isActive ? 'hover:bg-danger-50 text-surface-500 hover:text-danger-600' : 'hover:bg-success-50 text-surface-500 hover:text-success-600'
                                                    )}
                                                >
