...
function ImportProductCard({
    ...
    const statusConfig = {
        safe: { bg: 'bg-white', border: 'border-surface-200', text: 'text-success-700', label: 'Đủ hàng', badge: 'bg-success-100 text-success-700 border-success-200' },
        low: { bg: 'bg-warning-50', border: 'border-warning-200', text: 'text-warning-700', label: 'Sắp hết', badge: 'bg-warning-100 text-warning-700 border-warning-200' },
        out: { bg: 'bg-danger-50', border: 'border-danger-200', text: 'text-danger-700', label: 'Hết hàng', badge: 'bg-danger-100 text-danger-700 border-danger-200' },
    }[product.stockStatus];
    ...
    return (
        <div className={`relative rounded-2xl border overflow-hidden transition-all hover:shadow-md group flex flex-col ${statusConfig.bg} ${statusConfig.border}`}>
            ...
            <div className="relative aspect-[4/3] bg-surface-100 overflow-hidden">
                ...
                {inBatch && (
                    <div className="absolute top-2 left-2 bg-primary-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
            ...
            <div className="p-3 flex flex-col gap-2 flex-1">
                <div>
                    <p className="font-bold text-surface-800 text-sm leading-tight line-clamp-2">{product.companyCode || product.barcode || '—'}</p>
                    <p className="text-[14px] text-surface-800 mt-0.5 line-clamp-2">{product.name}</p>
                    {product.category && <p className="text-[14px] text-primary-600 mt-0.5">{product.category}</p>}
                </div>

                <div className="flex items-end justify-between mt-auto">
                    <div>
                        <p className="text-[10px] text-surface-400 uppercase tracking-wide">Tồn kho tổng</p>
                        <p className={`text-2xl font-black ${statusConfig.text}`}>
                            {product.currentStock}
                            <span className="text-xs font-normal text-surface-400 ml-1">{product.unit}</span>
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] text-surface-400">Tối thiểu</p>
                        <p className="text-sm font-semibold text-surface-600">{product.minStock}</p>
                    </div>
                </div>
                ...
                {product.minStock > 0 && (
                    <div className="w-full bg-surface-100 rounded-full h-1">
                        <div
                            className={`h-1 rounded-full transition-all ${product.stockStatus === 'safe' ? 'bg-success-500' : product.stockStatus === 'low' ? 'bg-warning-500' : 'bg-danger-500'}`}
                ...
                <button
                    ...
                    className={`w-full py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all ${inBatch
                        ...
                        : 'bg-gradient-to-r from-primary-600 to-cyan-600 hover:from-primary-700 hover:to-cyan-700 text-white shadow-sm shadow-primary-500/20'
                        }`}
                >
    ...
    const drawerContent = (
        <>
            ...
            <div className={`fixed inset-y-0 top-0 right-0 w-full max-w-md bg-white shadow-2xl z-[110] flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
                {/* Header */}
                <div className="p-5 border-b border-surface-100 flex items-center justify-between bg-gradient-to-r from-primary-600 to-cyan-600 text-white">
                ...
                <div className="px-5 py-3 border-b border-surface-100 bg-surface-50">
                    <label className="text-xs font-bold text-surface-600 flex items-center gap-1.5 mb-1.5">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-surface-200 rounded-xl p-2.5 text-sm outline-none focus:ring-2 focus:ring-primary-300"
                    />
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {batch.length === 0 ? (
                        ...
                        batch.map(item => (
                            <div key={item.productId} className="bg-surface-50 rounded-2xl p-3 border border-surface-200">
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold text-surface-800 text-sm truncate">{item.companyCode || item.barcode}</p>
                                        <p className="text-xs text-surface-400 truncate">{item.productName}</p>
                                    </div>
                                    <button onClick={() => onRemove(item.productId)} className="text-surface-400 hover:text-danger-500 p-1 rounded-lg transition-colors shrink-0">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <p className="text-[10px] text-surface-500 font-medium mb-1">Số lượng nhập</p>
                                        <div className="flex items-center gap-1">
                                            ...
                                            <input
                                                ...
                                                className="w-12 text-center text-sm font-bold bg-white border border-surface-200 rounded-lg py-1 outline-none focus:ring-2 focus:ring-primary-200"
                                            />
                                            <button
                                                onClick={() => onUpdateQty(item.productId, item.importQty + 1)}
                                                className="w-7 h-7 rounded-lg bg-surface-200 hover:bg-surface-300 flex items-center justify-center transition-colors"
                                    ...
                                    <div>
                                        <p className="text-[10px] text-surface-500 font-medium mb-1">Giá nhập (đ)</p>
                                        <input
                                            type="number" min={0}
                                            value={item.importPrice}
                                            onChange={e => onUpdatePrice(item.productId, Number(e.target.value) || 0)}
                                            className="w-full text-sm font-bold bg-white border border-surface-200 rounded-lg p-1.5 outline-none focus:ring-2 focus:ring-primary-200"
                                        />
                                    </div>
                                </div>

                                <p className="text-right text-xs text-surface-500 mt-2">
                                    Thành tiền: <span className="font-bold text-surface-700">{(item.importQty * item.importPrice).toLocaleString('vi-VN')}đ</span>
                                </p>
                            </div>
                        ))
                    )}
                ...
                {batch.length > 0 && (
                    <div className="p-4 border-t border-surface-100 bg-surface-50 space-y-3">
                        ...
                        <button
                            ...
                            className="w-full bg-gradient-to-r from-primary-600 to-cyan-600 hover:from-primary-700 hover:to-cyan-700 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50 shadow-md shadow-primary-500/20"
                        >
        ...
        <div className="space-y-6 mx-auto">
                ...
                titleChildren={
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
                        <div>
                            ...
                            <p className="text-surface-500 mt-1 text-sm">Chọn kho và lập lô nhập từ nhà cung cấp.</p>
                        </div>
                        {batch.length > 0 && (
                            <button
            ...
            {!loading && (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                        { label: 'Tổng sản phẩm', value: merged.length, color: 'text-surface-800' },
                        { label: 'Trong lô nhập', value: batch.length, color: 'text-primary-600' },
                        { label: 'Sắp hết hàng', value: lowCount, color: lowCount > 0 ? 'text-warning-600' : 'text-surface-800' },
                        { label: 'Hết hàng', value: outCount, color: outCount > 0 ? 'text-danger-600' : 'text-surface-800' },
                    ].map(card => (
                ...
                <div className="flex items-center gap-3">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-9 bg-surface-50 border border-surface-200 rounded-xl p-2.5 text-sm outline-none focus:ring-2 focus:ring-primary-300"
                        />
                    </div>
                    <button
                        onClick={() => setShowFilters(v => !v)}
                        className={`flex items-center gap-1.5 px-3 py-2.5 rounded-xl border text-sm font-medium transition-colors ${showFilters ? 'bg-primary-600 text-white border-primary-600' : 'bg-surface-50 text-surface-600 border-surface-200 hover:border-primary-300'}`}
                    >
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="appearance-none pl-3 pr-8 py-2.5 bg-surface-50 border border-surface-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-300 cursor-pointer"
                        >
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-surface-100">
                        {[
                            ...
                            <button key={f.key} onClick={() => setFilterStatus(f.key as typeof filterStatus)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors whitespace-nowrap ${filterStatus === f.key ? 'bg-primary-600 text-white' : 'bg-surface-100 text-surface-600 hover:bg-surface-200'}`}
                            >{f.label}</button>
                        ))}
                        <div className="w-px bg-surface-200 mx-1" />
                        <button onClick={() => setFilterCategory('')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${!filterCategory ? 'bg-surface-700 text-white' : 'bg-surface-100 text-surface-600 hover:bg-surface-200'}`}
                        >Mọi danh mục</button>
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setFilterCategory(cat)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${filterCategory === cat ? 'bg-surface-700 text-white' : 'bg-surface-100 text-surface-600 hover:bg-surface-200'}`}
                            >{cat}</button>
                        ))}
                    </div>
                )}
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-3">
                    ...
                    <p className="text-surface-400 text-sm">Đang tải danh sách sản phẩm...</p>
                </div>
            ) : filtered.length === 0 ? (
                <div className="text-center py-20 space-y-3">
                    <Package className="w-12 h-12 text-surface-300 mx-auto" />
                    <p className="text-surface-400 font-medium">Không tìm thấy sản phẩm nào</p>
                    <button onClick={() => { setSearch(''); setFilterStatus('all'); setFilterCategory(''); }} className="text-sm text-primary-600 hover:underline">Xóa bộ lọc</button>
                </div>
            ) : (
                <>
                    <p className="text-xs text-surface-400 font-medium">Hiển thị {filtered.length} / {merged.length} sản phẩm</p>
            ...
            {batch.length > 0 && (
                <button
                    onClick={() => setDrawerOpen(true)}
                    className="fixed bottom-6 right-6 z-30 bg-gradient-to-r from-primary-600 to-cyan-600 text-white rounded-2xl px-5 py-3.5 shadow-xl shadow-primary-500/30 flex items-center gap-3 hover:shadow-2xl hover:from-primary-700 hover:to-cyan-700 transition-all"
                >
                    <Truck className="w-5 h-5" />
                    <span className="font-bold text-sm">Lô nhập kho</span>
                    <span className="bg-white text-primary-700 font-black text-xs px-2 py-0.5 rounded-full min-w-[22px] text-center">
                        {batch.length}
                    </span>
                </button>
            )}
