...
function getPlacementKey(
    placement: Pick<ScannerPlacement, 'storeId' | 'counterId' | 'shiftDate' | 'shiftId'>,
) {
    return `${placement.storeId}:${placement.counterId}:${placement.shiftDate}:${placement.shiftId}`;
}
...
export default function ProductScannerPage() {
    ...
    const [countCheckpointType, setCountCheckpointType] = useState<ExternalCountCheckpointType>('SHIFT_OPENING');
    ...
    const refreshCountState = useCallback(async () => {
        ...
        try {
            const result = await getExternalCountStateAction(
                wmsWarehouseId,
                selectedLocationId,
                selectedPlacement?.shiftDate || getVietnamBusinessDate(),
                selectedPlacement?.shiftId,
                selectedPlacement?.shiftDate,
            );
    ...
    }, [selectedLocationId, selectedPlacement?.shiftDate, selectedPlacement?.shiftId, wmsWarehouseId]);
    ...
    const handleSubmitProduct = useCallback(async (product: PreloadedProduct, source: SubmitSource = 'manual') => {
        ...
        try {
            const result = await submitExternalScanAction({
                ...
                device_id: null,
                shift_id: selectedPlacement?.shiftId,
                shift_date: selectedPlacement?.shiftDate,
            });
            if (result.success) {
                playBeep(1800, 100);
                showToast.success(
    ...
    }, [authUser, countState, loadQueue, openCountSheet, selectedLocationId, selectedPlacement?.shiftDate, selectedPlacement?.shiftId, userDoc?.name, wmsWarehouseId]);
    ...
    const hasOptionalClosingEvidence = countState?.checkpoints.some(
        checkpoint =>
            checkpoint.checkpoint_type === 'OPTIONAL_CLOSING' &&
            (!checkpoint.shift_id || checkpoint.shift_id === selectedPlacement?.shiftId) &&
            (!checkpoint.external_operator_id || checkpoint.external_operator_id === authUser?.uid),
    ) ?? false;
        ...
        <div className="flex flex-col h-full gap-4">
                    ...
                    <div className="bg-white rounded-2xl border border-surface-100 shadow-sm p-3 shrink-0">
                        <div className="flex items-center justify-between gap-2 md:gap-4 text-xs">
                            ...
                            <div className="flex-1 flex items-center gap-2 min-w-0">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    {countState?.config.enabled ? (
                                        <button
                                            type="button"
                                            onClick={() => openCountSheet('SHIFT_OPENING')}
                                            disabled={!selectedLocationId || loadingCountState || openingCountSheet || !selectedPlacement.canStartOpeningCount || countState.access.current_shift_active}
                                        ...
                                        >
                                            {loadingCountState || (openingCountSheet && countCheckpointType === 'SHIFT_OPENING') ? (
                                                "Đang tải..."
                                            ) : !selectedPlacement.canStartOpeningCount ? (
                                                "Ca ngày trước"
                                            ) : beforeScanOpen ? (
                                                "Đã kiểm"
                                            ) : (
                            ...
                            <div className="flex-1 flex items-center gap-2 min-w-0">
                                <div className={cn(
                                    "flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold transition-all",
                                    hasOptionalClosingEvidence
                                        ? "bg-success-500 border-success-500 text-white"
                                        : "bg-surface-800 border-surface-800 text-white"
                                )}>
                                    {hasOptionalClosingEvidence ? "✓" : "3"}
                                </div>
                                <div className="min-w-0 flex-1">
                                    <span className="block font-bold text-surface-800 text-[11px] leading-tight">3. Cuối ngày</span>
                                    {countState?.config.enabled ? (
                                        <button
                                            type="button"
                                            onClick={() => openCountSheet('OPTIONAL_CLOSING')}
                                            disabled={!selectedLocationId || loadingCountState || openingCountSheet}
                                            className="mt-0.5 text-[10px] font-bold text-accent-600 hover:text-accent-700 active:scale-95 transition-transform block truncate text-left disabled:opacity-50"
                                        >
                                            {loadingCountState || (openingCountSheet && countCheckpointType === 'OPTIONAL_CLOSING') ? (
                                                "Đang tải..."
                                            ) : hasOptionalClosingEvidence ? (
                                                "Đã có minh chứng"
                                            ) : (
                ...
                <div className="w-full lg:w-96 flex flex-col min-h-0 bg-white rounded-2xl border border-surface-100 shadow-sm shrink-0 h-[40vh] lg:h-auto">
                    ...
                    {groupedQueue.length > 0 && (
                        <div className="shrink-0 border-t border-surface-100 bg-white p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
                            ...
                            >
                                {hasOptionalClosingEvidence ? <CheckCircle2 className="h-5 w-5" /> : <ClipboardCheck className="h-5 w-5" />}
                                {hasOptionalClosingEvidence ? 'Đã có minh chứng cuối ngày' : 'Kiểm kho cuối ngày (tùy chọn)'}
                            </button>
                        </div>
                    )}
                </div>
            ...
            {wmsWarehouseId && selectedLocationId && authUser && (
                <ExternalCountSheet
                    ...
                    shiftId={selectedPlacement.shiftId}
                    shiftDate={selectedPlacement.shiftDate}
