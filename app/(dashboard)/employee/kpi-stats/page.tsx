        ...
        <div className="space-y-6 mx-auto">
                ...
                titleChildren={
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
                        <div>
                            ...
                            <p className="text-surface-500 mt-1 text-sm">Xem điểm KPI của bạn theo tháng.</p>
                        </div>
                        {/* Month Picker */}
                        <div className="bg-white rounded-xl border border-surface-200 shadow-sm p-3 flex items-center gap-3 shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white rounded-xl border border-surface-200 p-4 shadow-sm">
                    <p className="text-xs text-surface-500 font-medium uppercase tracking-wider mb-1">Số ca đã chấm</p>
                    <p className="text-3xl font-black text-surface-800">{records.length}</p>
                </div>
                <div className="bg-white rounded-xl border border-surface-200 p-4 shadow-sm">
                    <p className="text-xs text-surface-500 font-medium uppercase tracking-wider mb-1">TB Tự đánh giá</p>
                    <p className={cn('text-3xl font-black', avgSelf >= 80 ? 'text-success-600' : avgSelf >= 50 ? 'text-warning-600' : 'text-danger-600')}>
                        {avgSelf}
                    </p>
                </div>
                <div className="bg-white rounded-xl border border-surface-200 p-4 shadow-sm">
                    <p className="text-xs text-surface-500 font-medium uppercase tracking-wider mb-1">TB Chính thức</p>
                    <div className="flex items-end gap-2">
                        <p className={cn('text-3xl font-black', avgOfficial >= 80 ? 'text-success-600' : avgOfficial >= 50 ? 'text-warning-600' : 'text-danger-600')}>
                            {avgOfficial}
                        </p>
                        {trend !== 0 && (
                            ...
                            <tbody className="divide-y divide-surface-100">
                                {records.sort((a, b) => b.date.localeCompare(a.date)).map(r => (
                                    <tr key={r.id} className="hover:bg-surface-50/50">
                                        <td className="p-3 font-medium text-surface-700">{r.date}</td>
                                        <td className="p-3 text-surface-600">{r.shiftId}</td>
                                        <td className="p-3 text-center">
                                            <span className={cn('font-bold', r.selfTotal >= 80 ? 'text-success-600' : r.selfTotal >= 50 ? 'text-warning-600' : 'text-danger-600')}>
                                                {r.selfTotal}
                                            </span>
                                        </td>
                                        <td className="p-3 text-center">
                                            {r.status === 'OFFICIAL' ? (
                                                <span className={cn('font-bold', r.officialTotal >= 80 ? 'text-success-600' : r.officialTotal >= 50 ? 'text-warning-600' : 'text-danger-600')}>
                                                    {r.officialTotal}
                                                </span>
                                        ...
                                        <td className="p-3 text-center">
                                            <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full border',
                                                r.status === 'OFFICIAL' ? 'bg-success-50 text-success-700 border-success-200' : 'bg-primary-50 text-primary-700 border-primary-200'
                                            )}>
