        ...
        <div ref={wrapperRef} className="relative">
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-300 transition-all"
                />
                {query && (
            ...
            {open && (
                <div className="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-xl max-h-64 overflow-y-auto">
                    {filtered.length === 0 ? (
                        <div className="p-4 text-sm text-slate-400 text-center">Không tìm thấy sản phẩm</div>
                    ) : (
                        filtered.map(p => {
                            ...
                            return (
                                ...
                                >
                                    <div className="min-w-0">
                                        ...
                                        <span className="text-slate-500 text-sm ml-2 truncate">{p.name}</span>
                                    </div>
                                    <div className="shrink-0 text-right">
                                        ...
                                        {isDisabled && (
                                            <span className="text-[10px] text-violet-500 block">Đã thêm</span>
                                        )}
                                    </div>
                                </button>
                            );
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-3 items-center">
                        ...
                        <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border-2 border-amber-200 p-5">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    <p className="text-[10px] font-bold text-amber-500 uppercase tracking-wider">Nguồn</p>
                                    <p className="font-bold text-amber-800 text-lg">Kho cửa hàng</p>
                                </div>
                            </div>
                        ...
                        <div className="hidden md:flex items-center justify-center">
                            <div className="w-12 h-12 rounded-full bg-gradient-to-r from-violet-500 to-purple-500 flex items-center justify-center shadow-lg shadow-violet-500/30">
                                <ArrowRight className="w-6 h-6 text-white" />
                            </div>
                        </div>
                        <div className="flex md:hidden justify-center -my-1">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-b from-violet-500 to-purple-500 flex items-center justify-center shadow-lg shadow-violet-500/30 rotate-90">
                        ...
                        <div className={`rounded-2xl border-2 p-5 transition-all ${selectedCounter
                            ...
                            <div className="flex items-center gap-3 mb-2">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedCounter ? 'bg-emerald-100' : 'bg-slate-200'
                                    ...
                                    <LayoutGrid className={`w-5 h-5 ${selectedCounter ? 'text-emerald-600' : 'text-slate-400'}`} />
                                </div>
                                <div className="flex-1">
                                    <p className={`text-[10px] font-bold uppercase tracking-wider ${selectedCounter ? 'text-emerald-500' : 'text-slate-400'
                                        }`}>Đích đến</p>
                                    <select
                                        value={selectedCounter}
                                        onChange={e => setSelectedCounter(e.target.value)}
                                        className={`font-bold text-lg bg-transparent outline-none cursor-pointer w-full ${selectedCounter ? 'text-emerald-800' : 'text-slate-400'
                                            }`}
                                    >
                            ...
                            <p className={`text-xs ${selectedCounter ? 'text-emerald-600/70' : 'text-slate-400'}`}>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                                ...
                                <span className="text-xs text-slate-400">
                                    Tổng: <span className="font-bold text-slate-700">{totalTransferUnits}</span> đơn vị
                                </span>
                            </div>
                                            ...
                                            <tr key={item.productId} className="border-b border-slate-100 hover:bg-slate-50/50">
                                                ...
                                                <td className="px-5 py-3">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-xs text-slate-400 line-clamp-1">{item.productName}</span>
                                                    </div>
                                                </td>

                                                {/* Current stock */}
                                                <td className="px-5 py-3 text-right">
                                                    <span className="text-slate-600 font-semibold">{item.currentStock}</span>
                                                ...
                                                <td className="px-5 py-3">
                                                    <div className="flex items-center justify-center gap-1.5">
                                                        ...
                                                        <input
                                                            ...
                                                            className="w-16 text-center text-sm font-bold bg-white border border-slate-200 rounded-lg py-1.5 outline-none focus:ring-2 focus:ring-violet-300"
                                                        />
                                                        <button
                                                            onClick={() => updateTransferQty(item.productId, item.transferQty + 1)}
                                                    ...
                                                    {item.transferQty >= item.currentStock && (
                                                        <p className="text-[10px] text-amber-500 text-center mt-1">Tối đa</p>
                                                    )}
                                                </td>
                            ...
                            <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/50">
                                <input
                                    ...
                                    className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-violet-300"
                                />
                            </div>
                        </div>
                    )}
                    ...
                    {transferItems.length === 0 && (
                        <div className="bg-white rounded-2xl border border-dashed border-slate-300 py-12 text-center">
                            ...
                            <p className="text-sm text-slate-400">Tìm kiếm và thêm sản phẩm vào danh sách xuất quầy</p>
                        </div>
                    )}

                    {/* ── ACTION FOOTER ── */}
                    {transferItems.length > 0 && (
                        <div className="fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
                            <div className="max-w-4xl mx-auto flex items-center justify-between px-5 py-4">
                                <div className="text-sm text-slate-600">
                                    <span className="font-bold text-violet-600">{transferItems.length}</span> sản phẩm
                                    {counterName && (
                                        <span className="text-slate-400 ml-1">→ <span className="font-semibold text-emerald-600">{counterName}</span></span>
                                    )}
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white rounded-xl font-bold text-sm flex items-center gap-2 transition-all disabled:opacity-50 shadow-md shadow-violet-500/20"
                                    >
                                        {submitting
