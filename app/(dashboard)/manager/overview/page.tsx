...
import { UserDoc, ScheduleDoc, CounterDoc, SettingsDoc } from '@/types';
import { getWeekStart, toLocalDateString } from '@/lib/utils';
import { Calendar, ChevronLeft, ChevronRight, Users, Clock, Store } from 'lucide-react';
...
export default function GlobalOverviewPage() {
    ...
    useEffect(() => {
        if (!userDoc || !['admin', 'manager'].includes(userDoc.role)) return;

        async function loadData() {
            setLoading(true);
            try {
                // 1. Load users (excluding admins, typically we schedule managers/employees)
                ...
                // 2. & 3. Load settings and counters
                ...
                // Auto-select first shift if shift mode is needed and none selected
                if (!selectedShift && setSnap.exists()) {
                ...
                // 4. Load schedules for the current week
                ...
                // Note: Not filtering with a composite query directly to avoid missing indices,
                // instead we fetch all and filter client-side for simplicity since week dataset is small.
                // In production, an index on `date` is highly recommended.
                const schedsSnap = await getDocs(collection(db, 'schedules'));
                let weekScheds = schedsSnap.docs.map(d => d.data() as ScheduleDoc);
                weekScheds = weekScheds.filter(s => s.date >= minDate && s.date <= maxDate);
        ...
        loadData();
    }, [currentWeekStart, userDoc]);

    if (!userDoc || (userDoc.role !== 'admin' && userDoc.canManageHR !== true)) {
    ...
    if (!settings && !loading) {
        return <div className="p-8">Vui lòng cấu hình hệ thống (Ca làm việc) trước.</div>;
    }
