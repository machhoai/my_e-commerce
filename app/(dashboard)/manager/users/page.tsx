...
import { UserDoc, EmployeeType } from '@/types';
...
export default function ManagerUsersPage() {
    ...
    useEffect(() => {
        ...
        // Managers only see users with role 'employee'
        const q = query(
            collection(db, 'users'),
            where('role', '==', 'employee')
        );
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // Sort locally to avoid needing a composite index on role + name
    ...
    const handleCreateOrUpdateUser = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editUid) {
                ...
                bodyPayload.role = 'employee'; // Hardcoded for manager create
            }
    ...
    const resetForm = () => {
        setNewName(''); setNewPhone(''); setNewType('PT');
                    ...
                    <table className="w-full text-sm text-left text-slate-600">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="py-12 text-center text-slate-400">Không tìm thấy nhân viên nào</td>
                                </tr>
                            ) : (
                                filteredEmployees.map((e) => {
                                    ...
                                    return (
                                        <tr key={e.uid} className={`hover:bg-slate-50/80 transition-colors group ${!isActive ? 'bg-slate-50/50' : ''}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`text-xs font-medium ${!isActive ? 'text-slate-400' : 'text-slate-600'}`}>
                                                    Nhân viên
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
