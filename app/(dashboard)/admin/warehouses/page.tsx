        ...
        <div className="space-y-6 pb-12">
            ...
            {showForm && (
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    <h2 className="font-bold text-slate-800 mb-4">{editingId ? 'Chỉnh sửa Kho' : 'Thêm Kho mới'}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="text-xs font-semibold text-slate-600 block mb-1">Tên Kho *</label>
                            <input value={formName} onChange={e => setFormName(e.target.value)}
                                className="w-full border border-slate-200 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-300"
                                placeholder="VD: Kho Tổng Bình Dương" />
                        </div>
                        <div>
                            <label className="text-xs font-semibold text-slate-600 block mb-1">Diện tích (m²)</label>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="py-16 text-center text-slate-400 text-sm">
                        <Warehouse className="w-10 h-10 mx-auto mb-3 text-slate-200" />
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {warehouses.map(wh => (
                                <tr key={wh.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-5 py-3">
                                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${wh.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                                    ...
                                    <td className="px-5 py-3 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button onClick={() => toggleActive(wh)}
                                                className={`p-2 rounded-lg transition-colors ${wh.isActive ? 'hover:bg-red-50 text-red-400 hover:text-red-600' : 'hover:bg-emerald-50 text-emerald-400 hover:text-emerald-600'}`}>
