...
export default function AIQuickChat({ isOpen, onClose }: AIQuickChatProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col justify-end pointer-events-auto" aria-modal="true" role="dialog">
            ...
            <div
                ref={sheetRef}
                className={`relative w-full bg-white rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-300 ${richMode ? 'max-h-[90vh]' : 'max-h-[80vh] lg:max-h-[50vh]'}`}
            >
                {/* Drag handle */}
