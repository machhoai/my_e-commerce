...
import {
    ...
    Pencil, User, MessageSquare, AtSign, Sparkles, Palette,
} from 'lucide-react';
...
export default function BulkEmailModal({
    ...
    const [ticketColor, setTicketColor] = useState(templateConfig.ticketColor || templateConfig.accentColor || '#22c55e');
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const body: Record<string, unknown> = {
                ...
                campaignId: campaign.id,
                templateConfig: { ...templateConfig, ticketColor },
                // Composition fields
                        ...
                        <div className="p-5 space-y-5">
                            <div className="flex items-center gap-3 bg-primary-50 border border-primary-100 rounded-xl p-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-surface-400 mt-0.5">Tùy chỉnh chủ đề, tên người gửi, màu vé và nội dung giới thiệu.</p>
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="text-xs font-bold text-surface-500 uppercase tracking-wide flex items-center gap-1.5 mb-2">
                                    <Palette className="w-3.5 h-3.5" /> Màu vé
                                </label>
                                <div className="space-y-3">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {TICKET_COLOR_PRESETS.map(p => (
                                            <button
                                                key={p.color}
                                                type="button"
                                                onClick={() => setTicketColor(p.color)}
                                                title={p.label}
                                                className={cn(
                                                    'w-9 h-9 rounded-xl border-2 transition-all active:scale-90 hover:scale-105',
                                                    ticketColor === p.color
                                                        ? 'border-surface-800 shadow-md ring-2 ring-surface-300'
                                                        : 'border-transparent hover:border-surface-300',
                                                )}
                                                style={{ background: p.color }}
                                            >
                                                {ticketColor === p.color && (
                                                    <CheckCircle2 className="w-4 h-4 text-white mx-auto drop-shadow" />
                                                )}
                                            </button>
                                        ))}
                                    </div>
                                    ...
                                    {/* ── Mini ticket-stub preview ──── */}
                                    <div className="bg-surface-50 rounded-xl border border-surface-200 p-4">
                                        <p className="text-[10px] font-bold text-surface-400 uppercase tracking-widest mb-2">Xem trước</p>
                                        <div
                                            className="bg-white rounded-xl overflow-hidden shadow-sm border border-surface-100"
                                            style={{ maxWidth: 340 }}
                                        >
                                            <div className="flex">
                                                {/* Left info */}
                                                <div className="flex-1 p-3 space-y-2">
                                                    <div className="flex items-center gap-2">
                                                        <div
                                                            ...
                                                            style={{ background: `${ticketColor}20`, border: `1.5px solid ${ticketColor}40` }}
                                                        >🎫</div>
                                                        <div>
                                                            <p className="text-xs font-bold text-surface-800 leading-tight">{templateConfig.title || campaign.name}</p>
                                                            <p className="text-[10px] font-mono text-surface-400">#DEMO-XXXX</p>
                                                        </div>
                                                    </div>
                                                    <div className="flex gap-1">
                                                        <span
                                                            className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold text-white"
                                                            style={{ background: ticketColor }}
                                                        >
                                                            ✓ Active
                                                        </span>
                                                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-surface-100 text-[9px] font-semibold text-surface-500">
                                                            📅 Valid
                                                        </span>
                                                    </div>
                                                    <div
                                                        className="px-2.5 py-1.5 rounded-lg text-[10px] font-bold text-white inline-block"
                                                        style={{ background: '#111827' }}
                                                    >🎫 Xem Voucher</div>
                                                </div>
                                                {/* Separator */}
                                                <div className="w-px my-2 border-l border-dashed border-surface-200 relative">
                                                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-surface-50" />
                                                    <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-surface-50" />
                                                </div>
                                                {/* QR placeholder */}
                                                <div className="flex items-center justify-center p-3 bg-surface-50/50">
                                                    <div className="w-14 h-14 rounded-lg border border-surface-200 bg-white flex items-center justify-center">
                                                        <svg viewBox="0 0 24 24" className="w-8 h-8 text-surface-300" fill="none" stroke="currentColor" strokeWidth="1.5">
