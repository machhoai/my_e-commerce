...
export default function ExcelImportModal({ getToken, onSuccess }: ExcelImportModalProps) {
            ...
            {open && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="p-6 border-b border-surface-100 flex items-center justify-between shrink-0">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-success-100 flex items-center justify-center">
                                    <FileSpreadsheet className="w-5 h-5 text-success-700" />
                                </div>
                                <div>
                                    <h2 className="text-lg font-bold text-surface-800">Nhập sản phẩm từ Excel</h2>
                                    <p className="text-xs text-surface-400">Tải template → điền dữ liệu → upload → xác nhận</p>
                                </div>
                            </div>
                            <button onClick={closeModal} className="text-surface-400 hover:text-surface-700 p-1 rounded-lg transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="px-6 pt-4 flex items-center gap-2 text-xs shrink-0">
                            {[
                                ...
                                <div key={s.id} className="flex items-center gap-2">
                                    <span className={`font-bold px-2.5 py-1 rounded-full ${step === s.id ? 'bg-primary-600 text-white'
                                        : (step === 'done' || (step === 'preview' && i === 0)) ? 'bg-success-100 text-success-700'
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-5">
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="bg-primary-50 border border-primary-200 rounded-2xl p-4 flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm font-bold text-primary-800">Bước 1: Tải file mẫu</p>
                                            <p className="text-xs text-primary-600 mt-0.5">
                                                Cột có dấu <span className="font-bold text-danger-500">*</span> là <strong>bắt buộc</strong>. Dùng đúng template để tránh lỗi parse.
                                            </p>
                                        </div>
                                        <button
                                            onClick={downloadTemplate}
                                            className="shrink-0 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-1.5 transition-colors"
                                        >
                                    ...
                                    {/* Column reference */}
                                    <div>
                                        <p className="text-xs font-bold text-surface-600 mb-2">Cột trong file Excel → trường dữ liệu</p>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                            {COLUMNS.map(col => (
                                                <div key={col.key} className={`rounded-xl p-2.5 border ${col.required ? 'bg-danger-50 border-danger-200' : 'bg-surface-50 border-surface-200'}`}>
                                                    <p className="text-[10px] text-surface-400 font-medium">{col.key}</p>
                                    ...
                                    {/* File picker */}
                                    <div>
                                        <p className="text-xs font-bold text-surface-600 mb-2">Bước 2: Upload file đã điền</p>
                                        <div
                                            onClick={() => fileInputRef.current?.click()}
                                            className="border-2 border-dashed border-surface-200 hover:border-primary-400 hover:bg-primary-50/40 rounded-2xl p-8 flex flex-col items-center gap-3 cursor-pointer transition-colors"
                                        >
                                            {parsing ? <Loader2 className="w-8 h-8 text-primary-500 animate-spin" /> : <Upload className="w-8 h-8 text-surface-300" />}
                                            <div className="text-center">
                                                <p className="text-sm font-bold text-surface-600">{parsing ? 'Đang đọc file...' : 'Nhấn để chọn file'}</p>
                                                <p className="text-xs text-surface-400 mt-0.5">Chấp nhận .xlsx và .xls</p>
                                            </div>
                                        </div>
                            ...
                            {step === 'preview' && (
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm font-bold text-surface-700">
                                                ...
                                                {invalidCount > 0 && (
                                                    <span className="ml-2 text-danger-600">— {invalidCount} hàng lỗi</span>
                                                )}
                                            </span>
                                        </div>
                                        <button
                                    ...
                                    {hasErrors && (
                                        <div className="p-3 bg-danger-50 border border-danger-200 rounded-xl flex items-start gap-2 text-sm text-danger-700">
                                        ...
                                        <table className="w-full text-xs border-collapse">
                                            <thead>
                                                <tr className="bg-surface-50">
                                                    <th className="p-2.5 text-left font-bold text-surface-500 border-b border-surface-200 whitespace-nowrap">#</th>
                                                    {COLUMNS.map(col => (
                                                        <th key={col.key} className="p-2.5 text-left font-bold border-b border-surface-200 whitespace-nowrap">
                                                            <span className={col.required ? 'text-surface-700' : 'text-surface-500'}>
                                                                {col.header}{col.required && <span className="text-danger-500 ml-0.5">*</span>}
                                                            </span>
                                                        </th>
                                                    ))}
                                                    {/* Status column */}
                                                    <th className="p-2.5 text-left font-bold text-surface-500 border-b border-surface-200 whitespace-nowrap">Trạng thái</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {previewRows.map((row, i) => (
                                                    <tr key={i} className={row.isValid ? (i % 2 === 0 ? 'bg-white' : 'bg-surface-50/50') : 'bg-danger-50'}>
                                                        <td className="p-2.5 text-surface-400 border-b border-surface-100">{i + 1}</td>
                                                        {COLUMNS.map(col => {
                                                            const val = String(resolveCell(row as Record<string, unknown>, col));
                                                            const missing = col.required && !val.trim();
                                                            return (
                                                                <td key={col.key} className={`p-2.5 border-b border-surface-100 whitespace-nowrap max-w-[150px] overflow-hidden text-ellipsis ${missing ? 'text-danger-400 font-bold' : 'text-surface-700'}`}>
                                                                    {val || (missing ? '—' : '')}
                                                                </td>
                                                            );
                                                        })}
                                                        <td className="p-2.5 border-b border-surface-100 whitespace-nowrap">
                                                            {row.isValid ? (
                                                                <span className="inline-flex items-center gap-1 text-success-700 font-bold">
                                                                    <CheckCircle2 className="w-3 h-3" /> Hợp lệ
                                                                </span>
                                                            ) : (
                                                                <span className="text-danger-600 font-medium">{(row.errors as string[]).join(', ')}</span>
                                                            )}
                                                        </td>
                                                    </tr>
                                                ))}
                            ...
                            {step === 'done' && (
                                <div className="flex flex-col items-center justify-center py-12 gap-4">
                                    ...
                                    <div className="text-center">
                                        <p className="text-lg font-bold text-surface-800">Nhập thành công!</p>
                                        <p className="text-sm text-surface-500 mt-1">{message.text}</p>
                                    </div>
                                    <button onClick={closeModal} className="mt-2 bg-success-600 hover:bg-success-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm transition-colors">
                                        Đóng
                                    </button>
                                </div>
                            )}
                        ...
                        {step === 'preview' && (
                            <div className="p-6 border-t border-surface-100 flex gap-3 shrink-0">
                                <button onClick={closeModal} className="flex-1 bg-surface-100 hover:bg-surface-200 text-surface-700 py-2.5 rounded-xl font-medium text-sm transition-colors">
                                    Hủy
                                </button>
                                <button
                                    ...
                                    className="flex-1 bg-gradient-to-r from-success-600 to-teal-600 hover:from-success-700 hover:to-teal-700 text-white py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-md shadow-success-500/20"
                                >
                                    {submitting ? (
    ...
    return (
        <>
            <button
                onClick={openModal}
                className="flex items-center gap-2 bg-success-600 hover:bg-success-700 text-white px-4 py-2.5 rounded-xl font-medium text-sm shadow-md transition-colors"
            >
